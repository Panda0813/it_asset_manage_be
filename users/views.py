...
from utils.permission import IsActiveUserOrReadOnly, IsActiveUser, IsNotLogin
from utils.ext_utils import REST_SUCCESS, REST_FAIL
...
from utils.conn_mssql import get_oa_users, get_oa_sections, get_oa_user_sections, get_sections_tree, trans_section_tree
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not distribute:
            if 'developer' not in user_roles:
                managerRoles = list(set(['assetManager']) & set(user_roles))
                tag_ls = []
        ...
        if belong_sys:
            queryset = queryset.filter(belong_sys=belong_sys)

        fuzzy_params = {}
        fuzzy_params['name'] = request.GET.get('name', '')
        fuzzy_params['role_code'] = request.GET.get('role_code', '')
...
class UserListGeneric(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if 'developer' in user_roles:
            pass
        elif list(set(['assetManager']) & set(user_roles)):
...
def query_sections_tree(request):
    try:
        ...
        trees = trans_section_tree(datas)
        return REST_SUCCESS(trees)
    except Exception as e:
        logger.error('查询失败, error: {}'.format(traceback.format_exc()))
        return REST_FAIL({'msg': '查询失败, error: {}'.format(str(e))})
...
def forget_pwd(request):
    ...
    if not email:
        return REST_FAIL({'msg': '邮箱不能为空'})
    user_qs = User.objects.filter(email=email)
    if not user_qs:
        return REST_FAIL({'msg': '系统中找不到该邮箱，请联系管理员'})
    user = user_qs.first()
    with transaction.atomic():
        save_id = transaction.savepoint()
        try:
            ...
            user.save()
            send_pwd_email([email])
            transaction.savepoint_commit(save_id)
        except Exception as e:
            transaction.savepoint_rollback(save_id)
            return REST_FAIL({'msg': '密码重置异常', 'error': str(e)})
    return REST_SUCCESS({"msg": "密码重置成功，请前往邮箱查看"})
